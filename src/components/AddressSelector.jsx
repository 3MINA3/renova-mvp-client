            ...
            <div>
              ...
              <span className="text-sm text-gray-500 dark:text-gray-400">
                {typeof user?.address === 'string' 
                  ? user.address 
                  : [user?.address?.city, user?.address?.street].filter(Boolean).join('، ')}
              </span>
            </div>
          </label>
        )}
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input 
                ...
                className="w-full pr-9 pl-3 py-2 text-sm border border-gray-200 dark:border-slate-700 dark:bg-slate-900 dark:text-white rounded-lg focus:ring-2 focus:ring-green-500 outline-none"
                placeholder="أدخل المدينة"
              />
            </div>
          </div>
          <div>
            <label htmlFor="newStreet" className="block text-xs font-bold text-gray-700 dark:text-gray-300 mb-1">الشارع *</label>
            <div className="relative">
              <MapPin className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
              <input 
                id="newStreet"
